...
def roomMenu(mainMenu):
    ...
    for num, room in enumerate (rooms, start = 1):
            typeWriter((str(num) + ". " + str(room)).center(160) + "\n\n")
            last_room = num
            if only_once == 0:
    ...
    typeWriter((str(last_room + 1) + ". Go back to Main Menu").center(160) + "\n\n")

    room_selection = int(input("Enter the number of the room you would like to investiagte: ".center(160) + "\n".center(160)))
...
def clueMenu(mainMenu):
    ...
    typeWriter((str(last_option + 1) + ". Go back").center(160) + "\n\n")
    clue_selection = int(input("Enter the number of the clue you would like to examine: ".center(160) + "\n".center(160))) 
    os.system("cls||clear")
    ...
    if dic.get(dic_item) == "game1":
        ...
        kitchenGame(roomMenu, mainMenu)

    elif dic.get(dic_item) == "game2":
        dic.pop(dic_item)
        room_clues.pop(clue_selection - 1)
        bathroomGame(roomMenu, mainMenu)

    elif dic.get(dic_item) == "game3":
        dic.pop(dic_item)
        room_clues.pop(clue_selection - 1)
        bedroomGame(roomMenu, mainMenu)

    elif dic.get(dic_item) == "game4":
        dic.pop(dic_item)
        room_clues.pop(clue_selection - 1)
        garageGame(roomMenu, mainMenu)
    
    elif dic.get(dic_item) == "game5":
        dic.pop(dic_item)
        room_clues.pop(clue_selection - 1)
        bathroomGame2(roomMenu, mainMenu)
