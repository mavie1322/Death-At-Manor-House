...
def kitchenGame(roomMenu, mainMenu):
    os.system("cls||clear")
...
def bathroomGame(roomMenu, mainMenu):
    os.system("cls||clear")
    ...
    if done:
        ...
        bathroomGame(roomMenu, mainMenu) if end == "y" else roomMenu(mainMenu)
...
def bathroomGame2(roomMenu, mainMenu):
    iteration = 0
    for i in range(len(questions2)):
        while True:
            try:
                answer = answers2[iteration] 
                saying = (input(questions2[iteration] + "..."))
                if saying == answer:
                    ...
                    print("Provide an answer without full stop")
                    print("Continue")
        iteration = iteration + 1
    typeWriter("Congratulations!, you find piece of wripped up paper with number 0 on it ")
    input()
    roomMenu(mainMenu)
...
def bedroomGame(roomMenu, mainMenu):
    os.system("cls||clear")
    global bedroom_message
    typeWriter(bedroom_message)
    typeWriter("--------------------\n")
    typeWriter("How well do you know maths? \n\n")
    score = 0
    for i in range(5):
        if ask_question() == True:
            ...
            os.system("cls||clear")
            typeWriter("\nCorrect!\n")
            typeWriter(f"Your score is {score}\n\n")
            typeWriter("---------------")
        else:
            os.system("cls||clear")
    ...
    if score >= 5:
        typeWriter("---------------\n")
        typeWriter("You've answered all questions correctly. CONGRATULATIONS!\n\n") 
        typeWriter(
            "The code to unlock the wardrobe is: 3275." 
            "\nYou open it and find that the wardrobe seems empty and looks hastely emptied."
            "\nAt the bottom there is a piece of paper with code number: 7")
        input("Continue?".center(160) + "\n".center(160))
        roomMenu(mainMenu)
    else:
        typeWriter("Try again. You must score 5 points to unlock the code!")   
        end = input("Play again? y/n")
        bedroomGame() if end == "y" else roomMenu(mainMenu)  
...
def garageGame(roomMenu, mainMenu):
