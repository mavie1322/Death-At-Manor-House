...
def guestMenu(mainMenu):
    ...
    for num, guest in enumerate (guests, start = 1):
            typeWriter((str(num) + ". " + str(guest)).center(160) + "\n\n")
            last_guest = num
            if only_once == 0:
    ...
    guest_selection = int(input("Enter the number of the person you would like to question: ".center(160) + "\n".center(160)))
...
def questionMenu(mainMenu):
    ...
    for num , question in enumerate (guests[guest_names[guest_selection - 1]].keys(), start = 1 ):
        typeWriter((str(num) + ". " + question).center(160) + "\n\n")
        guest_questions.append(question)
        last_question = num
  

    typeWriter((str(last_question + 1) + ". Goodbye " + guest_names[guest_selection - 1] ).center(160) + "\n\n") # prints another final question at the bottom of list e.g. 4. Go back 
    question_selection = int(input("Enter the number of the question you would like to ask: ".center(160) + "\n".center(160)))
    os.system("cls||clear")
